...
<main class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
  ...
  {:else}
     <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{#each data.trips as item}<a href={`/trips/${item.trip.id}`} aria-label={`Open trip ${item.trip.name}`} class="group"><Card class="h-full border-white/10 bg-white/[.025] transition hover:-translate-y-0.5 hover:border-sky-300/40"><CardHeader><p class="font-mono text-[10px] uppercase tracking-[0.2em] text-sky-300">{item.role} · active</p><CardTitle class="mt-3 text-xl tracking-[-0.04em]">{item.trip.name}</CardTitle></CardHeader><CardContent class="space-y-3 text-sm text-neutral-500">{#if item.trip.destination}<p>{item.trip.destination}</p>{/if}<p>{item.trip.startsOn ?? "Flexible dates"}{#if item.trip.endsOn} → {item.trip.endsOn}{/if}</p><div class="flex items-center justify-between border-t border-white/10 pt-3 font-mono text-[10px] uppercase tracking-wider"><span>{item.trip.currency} · {item.trip.timezone}</span><span class="font-semibold text-sky-300 transition group-hover:text-sky-200">Open trip →</span></div></CardContent></Card></a>{/each}</div>
  {/if}
</main>
