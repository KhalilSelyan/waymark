        ...
        {#each [...visibleDays, "unscheduled"] as day}
          ...
          <div class="space-y-3"><div class="flex items-center justify-between"><h2 class="text-lg font-semibold">{day === "unscheduled" ? "Unscheduled" : label(day)}</h2><span class="text-xs text-muted-foreground">{items.length} {items.length === 1 ? "item" : "items"}</span></div>
            ...
            {#each items as row}
              ...
              <Card><CardContent class="flex items-start justify-between gap-4 py-4"><div class="min-w-0"><p class="font-medium">{row.item.title}</p>{#if row.item.startsAt}<p class="text-sm text-muted-foreground">{time(row.item.startsAt)}{#if row.item.endsAt} – {time(row.item.endsAt)}{/if}</p>{/if}{#if row.place}<p class="text-sm text-muted-foreground">{row.place.name}</p>{/if}{#if row.item.notes}<p class="mt-2 whitespace-pre-wrap text-sm text-muted-foreground">{row.item.notes}</p>{/if}</div><div class="flex shrink-0 items-center gap-1"><Button variant="ghost" size="sm" disabled={saving || index === 0} onclick={() => move(row, -1)} aria-label="Move item up">↑</Button><Button variant="ghost" size="sm" disabled={saving || index === items.length - 1} onclick={() => move(row, 1)} aria-label="Move item down">↓</Button><Button variant="ghost" size="sm" onclick={() => edit(row)}>Edit</Button><Button variant="ghost" size="sm" onclick={() => archive(row.item.id)}>Archive</Button></div></CardContent></Card>
            {/each}
          </div>
        {/each}
      {/if}
