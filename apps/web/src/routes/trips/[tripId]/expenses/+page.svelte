<script lang="ts">
  ...
  let balances = $derived.by(() => {
    const values = new Map<string, { paid: number; netMinor: number }>(members.map((member) => [member.id, { paid: 0, netMinor: 0 }]));
    for (const item of expenses) {
      ...
      for (const balance of calculateBalances(item.expense.amountMinor, item.expense.payerMemberId, shares)) {
        const value = values.get(balance.memberId);
        if (value) value.netMinor += balance.netMinor;
      }
    }
    return members.map((member) => {
      const paid = values.get(member.id)?.paid ?? 0;
      const netMinor = values.get(member.id)?.netMinor ?? 0;
      return { ...member, paid, owed: paid - netMinor, netMinor };
    });
  });
  ...
  async function save() { fieldErrors = {}; const nextErrors: Record<string, string> = {}; if (!form.description.trim()) nextErrors.description = "Add a description."; if (!Number.isSafeInteger(amountMinor) || amountMinor <= 0) nextErrors.amount = "Enter an amount with up to two decimal places."; if (!form.payerMemberId) nextErrors.payerMemberId = "Choose who paid."; if (!form.participantIds.length) nextErrors.participants = "Choose at least one participant."; if (!form.occurredAt) nextErrors.occurredAt = "Choose when this expense happened."; if (form.splitType === "custom" && remaining !== 0) nextErrors.split = "Custom shares must equal the expense amount."; if (Object.keys(nextErrors).length) { fieldErrors = nextErrors; return; } saving = true; try { const shares = form.splitType === "equal" ? form.participantIds.map((memberId) => ({ memberId, amountMinor: 0 })) : form.participantIds.map((memberId) => ({ memberId, amountMinor: toMinor(form.customAmounts[memberId] || "") })); const input = { tripId, description: form.description.trim(), amountMinor, currency: trip.currency, payerMemberId: form.payerMemberId, splitType: form.splitType, shares, occurredAt: new Date(form.occurredAt).toISOString() }; if (editingId) await client.expenses.update({ id: editingId, ...input }); else await client.expenses.create(input); reset(); await refresh(); } catch (caught) { error = caught instanceof Error ? caught.message : "Expense could not be saved."; } finally { saving = false; } }
  async function remove(id: string) { try { await client.expenses.remove({ tripId, id }); expenses = expenses.filter((expense) => expense.expense.id !== id); removeTarget = null; await refresh(); } catch (caught) { error = caught instanceof Error ? caught.message : "Expense could not be deleted."; } }
  ...
  function settlementFor(suggestion: { fromMemberId: string; toMemberId: string; amountMinor: number }) { return settlementRows.find((row) => suggestionKey(row) === suggestionKey(suggestion)); }
  function settlementKey(suggestion: { fromMemberId: string; toMemberId: string; amountMinor: number }) { return settlementFor(suggestion)?.id ?? suggestionKey(suggestion); }
  async function toggleSettled(suggestion: { fromMemberId: string; toMemberId: string; amountMinor: number }) { const existing = settlementFor(suggestion); try { if (existing?.status === "settled") await client.expenses.settlements.markSuggested({ tripId, settlementId: existing.id }); else await client.expenses.settlements.markSettled({ tripId, ...suggestion }); await refresh(); } catch (caught) { error = caught instanceof Error ? caught.message : "Settlement status could not be updated."; } }
</script>
...
<section class="space-y-6">
  ...
  <div class="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_23rem]">
    ...
    <Card class="h-fit"><CardHeader><CardTitle>{editingId ? "Edit expense" : "Add expense"}</CardTitle></CardHeader><CardContent><form class="space-y-4" onsubmit={(event) => { event.preventDefault(); void save(); }}>
       <div class="space-y-2"><Label for="expense-description">Description</Label><Input id="expense-description" bind:value={form.description} required placeholder="Dinner" />{#if fieldErrors.description}<p class="text-xs text-destructive" role="alert">{fieldErrors.description}</p>{/if}</div>
       <div class="grid gap-3 sm:grid-cols-2"><div class="space-y-2"><Label for="expense-amount">Amount ({trip.currency})</Label><Input id="expense-amount" type="number" min="0.01" step="0.01" bind:value={form.amount} required />{#if fieldErrors.amount}<p class="text-xs text-destructive" role="alert">{fieldErrors.amount}</p>{/if}</div><div class="space-y-2"><Label for="expense-date">Date</Label><Input id="expense-date" type="datetime-local" bind:value={form.occurredAt} required />{#if fieldErrors.occurredAt}<p class="text-xs text-destructive" role="alert">{fieldErrors.occurredAt}</p>{/if}</div></div>
  ...
  <AlertDialog.Root open={Boolean(removeTarget)} onOpenChange={(open) => { if (!open) removeTarget = null; }}><AlertDialog.Content><AlertDialog.Header><AlertDialog.Title>Delete expense?</AlertDialog.Title><AlertDialog.Description>{removeTarget?.expense.description} will be removed and settlement suggestions will be recalculated.</AlertDialog.Description></AlertDialog.Header><AlertDialog.Footer><AlertDialog.Cancel>Cancel</AlertDialog.Cancel><AlertDialog.Action class="bg-destructive text-destructive-foreground hover:bg-destructive/90" onclick={() => removeTarget && void remove(removeTarget.expense.id)}>Delete expense</AlertDialog.Action></AlertDialog.Footer></AlertDialog.Content></AlertDialog.Root>
</section>
