...
export const GET: RequestHandler = async ({ request, cookies, params }) => {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      close = () => { if (closed) return; closed = true; clearInterval(heartbeat); unsubscribe(); leavePresence(params.tripId, access.member.id); publishRealtimeEvent({ tripId: params.tripId, actorMemberId: access.member.id, type: "presence.left", payload: { reason: "disconnect" } }); controller.close(); };
