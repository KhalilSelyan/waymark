...
import { getPresence, getStreamVersion, joinPresence, leavePresence, publishRealtimeEvent, subscribeRealtime, touchPresence } from "@waymark/api/realtime-hub";
  ...
  const stream = new ReadableStream({
    start(controller) {
      const send = (event: unknown) => controller.enqueue(encoder.encode(`data: ${JSON.stringify(event)}\n\n`));
       ...
       send({ type: "snapshot", tripId: params.tripId, streamVersion: getStreamVersion(params.tripId), objects, presence: getPresence(params.tripId) });
