...
const activeInvite = (token: string) =>
  and(
...
export const load: PageServerLoad = async ({ params }) => {
  const [invite] = await db.select({ trip: trips }).from(tripInvites).innerJoin(trips, eq(tripInvites.tripId, trips.id)).where(activeInvite(params.token)).limit(1);
...
export const actions: Actions = {
  default: async ({ params, request, cookies, url }) => {
    ...
    const [invite] = await db.select({ trip: trips, invite: tripInvites }).from(tripInvites).innerJoin(trips, eq(tripInvites.tripId, trips.id)).where(activeInvite(params.token)).limit(1);
    ...
    const data = Object.fromEntries(await request.formData());
     ...
     const parsedDisplayName = displayNameSchema.safeParse(data.displayName || parsed.data);
     if (!parsedDisplayName.success) return fail(400, { error: "Display name must be between 1 and 80 characters." });
    ...
    const existingGuestToken = cookies.get(`waymark_guest_${invite.trip.id}`);
     ...
     const [sameUsername] = await db.select({ id: tripGuests.id }).from(tripGuests).where(and(eq(tripGuests.tripId, invite.trip.id), eq(tripGuests.username, parsed.data))).limit(1);
     if (sameUsername) return fail(409, { error: "That username is already part of this trip." });

     const rawGuestToken = randomBytes(32).toString("base64url");
     const [guest] = await db.insert(tripGuests).values({ tripId: invite.trip.id, username: parsed.data, displayName: parsedDisplayName.data, guestTokenHash: createHash("sha256").update(rawGuestToken).digest("hex") }).returning();
